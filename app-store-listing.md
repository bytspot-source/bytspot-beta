# Bytspot — App Store Listing Copy

> Copy-paste ready for App Store Connect. Character limits noted.

---

## App Name (30 chars max)
```
Bytspot
```

## Subtitle (30 chars max)
```
Live Crowds, Parking & Spots
```

## Promotional Text (170 chars max — can be updated without a new build)
```
See live crowd levels, find parking faster, and discover the best spots around Atlanta Midtown — all in one clean app.
```

## Description (4000 chars max)

```
Bytspot shows you what's happening right now around Atlanta Midtown — live crowd levels, parking availability, and the best spots near you.

LIVE CROWD INTELLIGENCE
See how packed venues are before you go. Live crowd signals help you time your night and skip the longest waits. Check bars, restaurants, coffee shops, and entertainment venues at a glance.

SMART PARKING
Find available parking near any venue instantly. Compare prices, see walk times, and reserve your spot — all before you leave home. Save time and money with arrival guidance shaped around your plans.

SMART CONCIERGE
Ask Bytspot Concierge for help: "Where should I park for the Atlanta United game?" or "Find me a quiet coffee shop nearby." Sign in for tailored guidance; guests get instant local suggestions.

DISCOVER & EXPLORE
Swipe through curated venue cards with live photos, ratings, and crowd indicators. Filter by dining, nightlife, coffee, shopping, fitness, and more. Every card shows you what's happening right now, not last week.

PERSONALIZED FOR YOU
Bytspot learns your preferences over time. Your vibe quiz results, favorite cuisine types, and saved spots all shape your recommendations — no two users see the same feed.

SAVE & SHARE
Bookmark your favorite spots and share venues with friends using the native share sheet.

FEATURES:
• Live crowd levels for venues and parking
• Bytspot Concierge chat (signed-in) with instant local help for guests
• Live parking availability with pricing
• Venue discovery with swipeable cards
• Personalized feed based on your preferences
• Optional sign-in with guest browsing available
• Native haptic feedback and share sheet
• Works offline with cached data
• Dark mode optimized

Currently focused on Atlanta Midtown.
```

## Keywords (100 chars max, comma-separated)
```
parking,crowd,live,venues,Atlanta,nightlife,restaurants,concierge,discover,spots,Midtown,bars
```

## Primary Category
```
Navigation
```

## Secondary Category
```
Lifestyle
```

## Age Rating
```
12+ (Infrequent/Mild Alcohol, Tobacco, or Drug Use or References)
```
— Because the app shows bars/nightlife venues.

## Privacy Policy URL
```
https://bytspot.com/privacy
```

## Support URL
```
https://bytspot.com/support
```

## Marketing URL (optional)
```
https://bytspot.com
```

## Copyright
```
© 2026 Bytspot
```

---

## What's New in This Version (for updates)
```
Native SwiftUI update with a faster, cleaner Home experience, improved parking/discovery flow, and continued App Clip access support.
```
