...
const checks = [
  ...
  ['Main App launches pure native SwiftUI root', appDelegate.includes('UIHostingController(rootView: NativeScenePhaseRoot(lifecycle: lifecycle, content: BytspotNativeAppRoot()))') && appDelegate.includes('content.environment(\\.scenePhase, lifecycle.phase)') && !appDelegate.includes('CAPBridgeViewController') && !appDelegate.includes('ApplicationDelegateProxy')],
  ...
  ['Party Pass posters report the container size and fit inside it', clipContentViewCode.includes('struct ClipContainedFillImage') && clipContentViewCode.includes('frame(width: proxy.size.width, height: proxy.size.height)') && (clipContentView.match(/ClipContainedFillImage\(url: poster, contentMode: \.fit\)/g) ?? []).length >= 2 && !clipContentViewCode.includes('.overlay(AsyncImage(url: poster)')],
