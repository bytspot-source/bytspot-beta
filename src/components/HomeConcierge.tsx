...
export function HomeConcierge({ isOpen, onClose, venues, onVenueSelect }: HomeConciergeProps) {
      ...
      {isOpen && (
        <>
          ...
          >
                      ...
                      <div className="mt-3 space-y-2">
                        {m.venues.map(v => (
                          <button key={v.id ?? v.name}
                            onClick={() => { onVenueSelect(v); onClose(); }}
            ...
            {/* Input */}
            <div className="flex items-center gap-2 px-4 py-3 border-t border-white/10 flex-shrink-0" style={{ paddingBottom: 'calc(12px + env(safe-area-inset-bottom))' }}>
              <input
                type="text"
                value={input}
