...
test('renders ordinary Party Pass URLs on the web and preserves explicit App Clip handoffs', () => {
  const url = 'https://bytspot.app/party/party-1?handoff=1';
  const context = nativeHandoffContext(url);

  assert.equal(nativeAppClipArgumentFor(url), url);
  assert.equal(shouldBlockLegacyPwaFallback(url), true);
  assert.equal(shouldBlockLegacyPwaFallback('https://bytspot.app/party/party-1'), false);
...
test('service worker bypass classifier stays aligned for handoff navigation URLs', () => {
  ...
  assert.equal(isNativeHandoffURL(new URL('https://bytspot.app/party/party-1')), false);
  assert.equal(isNativeHandoffURL(new URL('https://bytspot.app/party/party-1?handoff=1')), true);
